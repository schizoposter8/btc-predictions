...
import { DEC31_CLOSE, ATH, MEMBER_COLORS } from "../constants";
import { formatPrice, formatK } from "../utils";

export default function PriceChart({ predictions, members, currentPrice, monthlyPrices }) {
  ...
  const allPrices = [
    ...monthlyPrices.filter(m => m.price).map(m => m.price),
    ...predValues,
  ...
  const actualPrices = monthlyPrices
  ...
  const lastHardcodedIndex = monthlyPrices.reduce((last, m, i) => (m.price ? i : last), 0);
  if (currentPrice && currentMonth > lastHardcodedIndex) {
    actualPrices.push({
      x: xScale(currentMonth),
      y: yScale(currentPrice),
      price: currentPrice,
      month: monthlyPrices[currentMonth] ? monthlyPrices[currentMonth].month : "Now"
    });
  }
  ...
  return (
    <div style={{ position: "relative", width: "100%", overflowX: "auto" }}>
      ...
      >
        ...
        {/* Price dots - colored by position vs previous month */}
        {actualPrices.map((p, i) => {
          const prevPrice = i > 0 ? actualPrices[i - 1].price : p.price;
          return (
            <circle key={i} cx={p.x} cy={p.y} r="4"
              fill={p.price >= prevPrice ? "#00d68f" : "#ff4757"}
